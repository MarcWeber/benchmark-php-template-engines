...
";
  ...
  static public function hamlToPHPStr($str, $options, $filename = '', $func_name = null){
    // this is only used when not using service, see hamlToPHP below
    if (defined('HAML_SERVICE_URL'))
      throw new Exception('HamlUtilities::hamlToPHPStr should never have been called when using HAML TO PHP service');
	  ...
	  if ($this->forceUpdate 
		  || (
			...
			if (!file_put_contents($c, $this->hamlToPHP(file_get_contents($h), $h)))
                          throw new Exception('failed writing to : '.$c);
	  }
